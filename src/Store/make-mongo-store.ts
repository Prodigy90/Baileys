...
export default ({
	...
	const bind = (ev: BaileysEventEmitter) => {
		...
		ev.on('contacts.update', async(updates) => {
			for(const update of updates) {
				...
				if(contact) {
					...
					logger.debug('got update fornon-existent contact')
				}
			}
		})
		...
		ev.on('chats.update', async(updates) => {
			...
			for(const update of updates) {
				...
				if(!chat) {
					logger.debug('got update fornon-existant chat')
				}
			}
		})
		...
		ev.on('messages.update', (updates) => {
			for(const { update, key } of updates) {
				...
				if(!result) {
					logger.debug('got update fornon-existent message')
				}
			}
		})
		...
		ev.on('groups.update', (updates) => {
			for(const update of updates) {
				...
				if(groupMetadata[id]) {
					...
					logger.debug(
						{ update },
						'got update fornon-existant group metadata'
					)
				}
			}
		})
	 ...
	 * @return {Promise<Chat|null>} A promise that resolves to the chat object iffound, or null ifnot found.
	 */
	...
	return {
		...
		labelAssociations,
		bind,
		/** loads messages from the store, ifnot found -- uses the legacy connection */
		loadMessages: async(
			jid: string,
			count: number,
		...
		/**
		 * Get all available labels forprofile
		 *
