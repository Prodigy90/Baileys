...
const makeSocket = (config) => {
  ...
  if (config.mobile && url.protocol !== "tcp:") {
    url = new url_1.URL(
      `tcp://${Defaults_1.MOBILE_ENDPOINT}:${Defaults_1.MOBILE_PORT}`
    );
  }
  if (
    !config.mobile &&
  ...
  const sendNode = async (frame) => {
    try {
      ...
      // Await the result of encodeBinaryNode
      const buff = await WABinary_1.encodeBinaryNode(frame);

      // Call sendRawMessage with the resolved buffer
      return sendRawMessage(buff);
    } catch (error) {
      console.error("Error in sendNode:", error);
    }
  };
